...
for i in range(len(app_list)):
    ...
    if title_path:
        ...
        game_mode_path = driver.find_elements_by_xpath("//*[contains(text(), '게임 모드')]/following-sibling::div/span")
        if game_mode_path:
            game_mode_list = game_mode_path[0].text.split(', ')
            game_mode.extend(game_mode_list)
            game_mode_set |= set(game_mode_list)

        play_mode_path = driver.find_elements_by_xpath("//*[contains(text(), '지원되는 플레이 모드')]/following-sibling::div/a/span")
        if play_mode_path:
            play_mode_list = play_mode_path[0].text.split(', ')
            play_mode.extend(play_mode_list)
            play_mode_set |= set(play_mode_list)

        category_path = driver.find_elements_by_xpath("//*[contains(text(), '카테고리')]/following-sibling::div/span")
        if category_path:
            category = category_path[0].text
            category_set.add(category)

        genre_path = driver.find_elements_by_xpath("//*[contains(text(), '장르')]/following-sibling::div/span")
        if genre_path:
            genre_list = genre_path[0].text.split(', ')
            genre.extend(genre_list)
            genre_set |= set(genre_list)

        languages_path = driver.find_elements_by_xpath("//*[contains(text(), '언어')]/following-sibling::div/span")
        if languages_path:
